...
class ImageProcessor:
    ...
    def select(self, iterable_names, title, repeat_intro=True):

        if repeat_intro:
            print(title)
            for index, name in enumerate(iterable_names):
        ...
        if selected_action < 0 or selected_action > len(iterable_names):
            return None
        selected_action -= 1

        return iterable_names[selected_action]
        ...
        for y in range(0, height, tile_height):
            for x in range(0, width, tile_width):
                ...
                if image_regions[i].shape[1] != tile_width or image_regions[i].shape[0] != tile_height:
                    i += 1
                    continue
                closest_region_match = self.find_best_match_for_region(image_regions[i])
                mosaic[y:y + tile_height, x:x + tile_width] = closest_region_match
                i += 1
...
if __name__ == '__main__':
    ...
    cv2.imwrite(f"dest/slow-mosaic-picture-{input_image.split('.')[0]}-{tiles_size[0]}x{tiles_size[1]}.jpg", mosaic_image)
